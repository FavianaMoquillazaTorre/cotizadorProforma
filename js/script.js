...
function totalCost(event){
    ...
    price_field_content = price_field.innerText.replace('S/', '')
    total_field.children[0].innerText = 'S/' +  quantity.value * price_field_content
    grandTotal()
...
function grandTotal(){
    ...
    for(let i = 0; i < all_total_fields.length; i++){
        all_prices = Number(all_total_fields[i].innerText.replace('S/', ''))
        total+=all_prices
    }
    grand_total.children[0].innerText = "S/"+total
